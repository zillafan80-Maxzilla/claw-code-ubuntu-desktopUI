...
async function detectModelConfig(root) {
  ...
  const baseConfig = {
    ...
    tool_call_style:
      ...
      "auto",
    locale:
      process.env.CLAW_DESKTOP_LOCALE ||
      envFromUi.CLAW_DESKTOP_LOCALE ||
      persisted.locale ||
      "en",
  };
...
function normalizeModelConfig(config) {
  const normalized = {
    ...
    locale: String(config.locale || "en").trim().toLowerCase(),
  };
  if (!normalized.tool_call_style) {
    normalized.tool_call_style = "auto";
  }
  if (!["en", "ja", "ko", "zh"].includes(normalized.locale)) {
...
function writeEnvFile(root, config) {
  ...
  const content = [
    ...
    `export CLAW_DESKTOP_LOCALE="${escapeShell(config.locale)}"`,
    "",
  ].join("\n");
...
function printSummary(result) {
  ...
  console.log(`语言: ${result.modelConfig.locale}`);
