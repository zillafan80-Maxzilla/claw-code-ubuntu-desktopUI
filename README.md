...
10. 自 `1.2` 起的用户打开桌面窗口后，会自动检查是否存在更新；如发现 `1.5` 或更高版本，会弹出更新提示并可直接执行升级。
...
10. Users coming from `1.2` will see an upgrade prompt on startup when `1.5` or newer is available, and can update in place.
